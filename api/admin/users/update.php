<?php
// api/admin/users/update.php - Update user (Admin only)
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT' || $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($data->user_id) || !isset($data->email) || !isset($data->role)) {
    ...
    // Validate role
    if (!in_array($role, ['ADMIN', 'MEDICAL_PROFESSIONAL', 'USER'])) {
        http_response_code(400);
        echo json_encode([
            "success" => false,
    ...
    try {
        ...
        $updateQuery = "UPDATE users 
                        SET email = :email, first_name = :first_name, last_name = :last_name, role = :role 
                        WHERE id = :user_id";
